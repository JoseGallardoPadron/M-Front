...
export class AuthService {
        ...
        .then((userCredential) => {
            ...
            return updateProfile(user, { displayName: username }).then(() => {
                return user.getIdToken().then((token) => {
                    ...
                    return this.http.post('https://8080-vallegrande-microservic-jgsyheistd0.ws-us118.gitpod.io/api/auth/register', {
  ...
  verifyTokenWithBackend(): Observable<any> {
    return from(this.getToken()).pipe(
      switchMap((idToken) => {
        if (idToken) {
          return this.http.get('https://8080-vallegrande-microservic-jgsyheistd0.ws-us118.gitpod.io/api/auth/verifyToken', {
